...
extern SemaphoreHandle_t exclusiveAccessMutex;  // Links the Mutex used in the DRV_CAN_WRITE to the mutex defined in the main.c
...
void DRV_CAN_INIT(spi_device_handle_t* spi) {
    ...
    while (!v) {
        ...
        txfConfig.TxAttempts =0;       // only one attempt
...
void DRV_CAN_WRITE(spi_device_handle_t* spi, uint8_t* txd, uint16_t id, CAN_DLC length) {
    ...
    txObj.bF.id.SID = id;           // what is this ID? would it prevent things from being read?
    txObj.bF.id.EID = 0;
    ...
    // When tx flags is not even it means that there is something to be pushed out
    ...
    // 17 means transmit exhausted that there is an interrupt pending. 
    // 13 means that there is an empty FIFO 
